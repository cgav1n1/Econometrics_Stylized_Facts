...
\noindent Figure~\ref{fig:Rolling_std_dev} illustrates the relationship between Amazon's daily log returns and changes in the VIX over the period from January 21, 1999, to October 16, 2024. Each blue point represents a specific day, capturing the joint movement of Amazon's log returns and VIX changes. This scatter plot is consistent with the findings from Figure 11, which highlight a negative correlation between returns and volatility. The clustering of volatility around the OLS regression line further supports the presence of a correlation between Amazon's returns and its volatility.
